...
unsigned char MSFunc3(int addr,int len)
{
	...
	if (addr<14 && addr + len<=14)	
	{
		...
		{
			rsBuf[i+3] = *(((char*)&Value) + addr*2 + i + 1);
			rsBuf[i+4] = *(((char*)&Value) + addr*2 + i);
		}
		return rsBuf[2]+3;
	}
	else if (addr>=0x4000 && addr+len<= 0x4010)
	{
		...
		//jmemcpy(((unsigned char*)&sValue+addr), &rsBuf[3],len);
		for(i=0;i<rsBuf[2];i+=2)
		{
			rsBuf[i+3] = *(((unsigned char*)&sValue) + addr*2 + i + 1);
			rsBuf[i+4] = *(((unsigned char*)&sValue) + addr*2 + i);
		}
		return rsBuf[2]+3;
	}
	return 0;
...
unsigned char MSFunc6(int addr,int value)
{
	...
	{
		addr -= 0x4000;
		*(((unsigned int *)&sValue)+addr) = swap(v);
		i2cWriteStr(addr*2,(unsigned char*)(((unsigned char*)&sValue)+addr*2),2);
	}
	return RcvCnt;
}	
...
unsigned char MSFunc16(int addr,int len)
{
	...
	if (addr>=0x4000 && addr+len<0x4010)
	{
		...
		{
			*(((unsigned char*)&sValue) + addr*2 + i + 1) = rsBuf[i+7];
			*(((unsigned char*)&sValue) + addr*2 + i) = rsBuf[i+8];
		}
		i2cWriteStr(addr*2,(((unsigned char*)&sValue) + addr*2) ,rsBuf[6]); 
		return 6;
	}
	return 0;
}
...
void ModbusSlave()
{
	...
	if (1==Rcved)
	{
		...
		if (crc16(rsBuf,RcvCnt)==0)
		{
			...
			{
				case 1:case 2: 
					SndCnt = MSFunc1(Addr,Len);
					break;
				case 3:case 4: 
					SndCnt = MSFunc3(Addr,Len);
					break;
				case 5:
					SndCnt = MSFunc5(Addr,Len);
					break;
				case 6: 
				...
				case 15: break;
				case 16: 
					SndCnt = MSFunc16(Addr,Len);
					break;
			}
		}
		if (SndCnt)
		{
			crc = crc16(rsBuf,SndCnt);
			rsBuf[SndCnt++]=crc;
			rsBuf[SndCnt++]=crc>>8;
		}
		//SndCnt = RcvCnt;
