...
def check_encoding(view, encoding, confidence):
    view.set_status('origin_encoding', ('Detected {0} with {1:.0%} confidence'.format(
        encoding, confidence)) if encoding else 'Encoding can not be detected')
...
def init_encoding_vars(view, encoding, run_convert=True, stamp=None, detect_on_fail=False):
    ...
    if run_convert:
        ...
        # view.run_command('convert_to_utf8', {'stamp': stamp})  # 'detect_on_fail': detect_on_fail,
        # jwz:modified
